...
During BF, the counterfactual correctly projects the trend-shifted November level (~1,800 USD/day) but cannot replicate the sharp BF acceleration above it. Actual revenue spikes to 4,241 USD/day. The model attributes almost the entire spike to the campaign.

Why does the counterfactual under-estimate the spike? The BSTS seasonal component partially learned from BF 2024 — it knows late November is elevated. But the weekly seasonal bin averages the BF days with surrounding days, so the model learns a moderate seasonal lift rather than a sharp one. What remains in the gap above 1,800 USD/day is a mix of true campaign contribution and organic BF demand that exceeded the smoothed seasonal component the model learned.

The gap also cannot be cleanly decomposed. The model learned organic BF demand at the 2024 level. Whatever organic demand grew between 2024 and 2025 — due to baseline growth, stronger consumer intent, or any other factor — lands in the gap alongside the campaign effect. Without donor channels observing organic demand directly at the 2025 level, there is no way to separate the two.

The result is something close to a trend-adjusted YoY — total BF performance vs a modeled baseline that accounts for growth. Better than raw YoY, but still not incrementality.
