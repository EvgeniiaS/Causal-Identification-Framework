...
During BF, the counterfactual sits at ~1,800 USD/day — the correctly projected trend-shifted pre-BF level. Actual revenue spikes to 4,241 USD/day. The gap from 1,800 USD/day to 4,241 USF/day contains two things: the organic BF demand spike and the paid campaign contribution. The model attributes the entire gap to the campaign.

The reason the gap is so large is that the counterfactual stays flat at the pre-BF level rather than anticipating any BF elevation. The BSTS seasonal component partially learned from BF 2024 — it knows late November is elevated — but the weekly seasonal bin averages BF days with surrounding days, producing a moderate seasonal lift rather than a sharp spike. Had the model correctly forecast a BF spike based on 2024, that predicted spike would itself contain 2024 organic demand and 2024 paid campaign contribution — so the remaining gap would reflect only the year-over-year change in each, not a clean read on either. The inability to replicate the spike is what causes the full 2025 BF organic demand to land in the gap alongside the true paid increment.

The result is something close to a trend-adjusted YoY — total BF performance above the pre-BF modeled level, with organic demand conflated with campaign contribution throughout.
