...
During BF, the counterfactual sits at ~1,800 USD/day — the correctly projected trend-shifted November baseline. Actual revenue spikes to 4,241 USD/day. The gap from 1,800 to 4,241 contains two things: the organic BF demand spike and the paid campaign contribution. The model attributes the entire gap to the campaign.

The reason the gap is so large is that the counterfactual stays flat at the November baseline rather than anticipating any BF elevation. The BSTS seasonal component partially learned from BF 2024 — it knows late November is elevated — but the weekly seasonal bin averages BF days with surrounding days, producing a moderate seasonal lift rather than a sharp spike. Had the model correctly forecast a 2025 BF spike in the counterfactual, the gap would shrink to only the organic demand above what the model predicted, plus the paid campaign contribution. The failure to replicate the spike is what causes the full organic demand wave to land in the gap alongside the true paid increment.

The result is something close to a trend-adjusted YoY — total BF performance above the modeled November baseline. Better than raw YoY, but the gap still contains organic demand that belongs to the counterfactual, not to the campaign.
